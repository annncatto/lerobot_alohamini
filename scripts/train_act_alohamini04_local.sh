...
DATA=/home/anncatto/alohamini_gui/datasets/lerobot/local/alohamini_04
RUN=${RUN:-act_alohamini04_fixed_left_joint_loss_fullspeed_$(date +%Y%m%d_%H%M%S)}
...
exec "$PYTHON" -u -m lerobot.scripts.lerobot_train \
  --dataset.repo_id=local/alohamini_04 \
  --dataset.root="$DATA" \
