...
def extract_images_from_raw_observation(
    ...
    image = lerobot_obs[camera_key]
    if isinstance(image, JPEGImage):
        import cv2
        import numpy as np

        image = cv2.imdecode(np.frombuffer(image.data, dtype=np.uint8), cv2.IMREAD_COLOR)
        if image is None:
            raise ValueError(f"Could not decode JPEG observation {camera_key!r}")
...
def prepare_raw_observation(
    ...
    image_dict = {
        rename_map.get(key, key): resize_robot_observation_image(
            extract_images_from_raw_observation(lerobot_obs, key),
...
class JPEGImage:
    """A JPEG-compressed HWC observation image transported over gRPC."""
